...
question2(X2):-	write("Is your character somewhat moral?"),nl,
				write("2. Yes, they are a good person"),nl,
...
%member(+List, +Element) - check if element is in list
member([El|_], El).
member([_|T], El) :- member(T, El).

%play/0
play:-	question1(X1),

	question2(X2),
	findall(X, (dead(X,X1),moral(X,X2)), Match1),
    	check_if_guessed(Match1),

	question3(X3),
	findall(X, (member(Match1,X),criminal(X,X3)), Match2),
    	check_if_guessed(Match2),

	question4(X4),
	findall(X, (member(Match2,X),lawyer(X,X4)), Match3),
    	check_if_guessed(Match3),

	question5(X5),
	findall(X, (member(Match3,X),woman(X,X5)), Match4),
    	check_if_guessed(Match4),

	question6(X6),
	findall(X, (member(Match4,X),mexican(X,X6)), Match5),
    	check_if_guessed(Match5),

	branch(Match5,X6).

%branch(+Match,+X6) - ask next question depending on answer to sixth 
branch(Match,X6):-(X6=:=0 ->question7(X7),
		findall(X, (member(Match, X),role(X,X7)), Match6),
    		check_if_guessed(Match6);
		X6=:=1 ->question8(X8),
		findall(X, (member(Match, X),soft_skills(X,X8)), Match7),
    		check_if_guessed(Match7)).

   

% check_if_guessed(+MatchList) - check if a given list contains only one element and if so write this element
check_if_guessed(MatchList) :- length(MatchList, MatchCount), (MatchCount =:= 1 -> [Answer | _] = MatchList, write(Answer),fail;true).